...
interface HomeWishCardProps {
  ...
  isAchievementCard?: boolean  // 新しいプロパティを追加
}

export function HomeWishCard({ wish, isUnknownDateCard = false, isAchievementCard = false }: HomeWishCardProps) {
  ...
  const handleAchievementUpdate = async () => {
    const { error } = await updateCustomWish(wish.custom_wish_id, {
      ...
      customwish_image_url: achievementForm.customwish_image_url,
      achievement_date: achievementForm.achievement_date ? 
        format(achievementForm.achievement_date, 'yyyy-MM-dd') : null,
      thoughts: achievementForm.thoughts,
      updated_at: new Date().toISOString()
    });
      ...
      return (
        <Card>
            ...
            <form className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  id="image"
                  type="url"
                  value={achievementForm.customwish_image_url}
                  onChange={(e) => setAchievementForm(prev => ({ ...prev, customwish_image_url: e.target.value }))}
                  placeholder="https://..."
                />
              </div>
              ...
              <div className="space-y-2">
                <Label>感想</Label>
                <Textarea
                  value={achievementForm.thoughts}
                  onChange={(e) => setAchievementForm(prev => ({ ...prev, thoughts: e.target.value }))}
          ...
          <CardFooter className="flex justify-end space-x-2">
            ...
            <Button
              onClick={handleAchievementUpdate}
        ...
        <div className="p-4 space-y-3">
          <div className="space-y-2">
            ...
            <div className="space-y-4">
              <div>
                <h4 className="text-sm font-semibold text-gray-700 mb-2">達成日</h4>
                <p className="text-gray-600">
                  {wish.achievement_date ? format(new Date(wish.achievement_date), 'yyyy年MM月dd日') : '未設定'}
                </p>
              </div>
              <Separator />
              <div>
                <h4 className="text-sm font-semibold text-gray-700 mb-2">感想</h4>
